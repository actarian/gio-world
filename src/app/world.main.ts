/* global window, document, console, THREE */
...
class World {
	...
	constructor(options: Object) {
		let node, scene, camera, controls, renderer, clock, fov, near, far, width, height;
		this.options = {
			id: 'scene',
...
class Geopos {
	...
	toVector() {
		return Geopos.toVector(this);
	}
}
let shader, uniforms, group, sphere, stars, starfield, lights = [];
let world = new World({
	id: 'world',
	objects: function (world: World) {
		group = new THREE.Group();
		group.rotation.y = -Math.PI / 180 * 40;
		let diffusePath = 'https://rawgit.com/actarian/plausible-brdf-shader/master/textures/mars/2048x1024/diffuse.jpg';
		let normalPath = 'https://rawgit.com/actarian/plausible-brdf-shader/master/textures/mars/2048x1024/normal.jpg';
		let bumpPath = 'https://rawgit.com/actarian/plausible-brdf-shader/master/textures/mars/4096x2048/bump.jpg';
		let specularPath = 'https://rawgit.com/actarian/plausible-brdf-shader/master/textures/mars/2048x1024/specular.jpg';
		shader = new THREE.ShaderMaterial({
			uniforms: THREE.UniformsUtils.merge([
				THREE.UniformsLib['lights'], {
					...
					//
					ambient: { value: new THREE.Color(0x111122) },
		...
		loader.crossOrigin = '';
		loader.load(diffusePath, function (texture) {
			// texture.mapping = THREE.SphericalReflectionMapping;
			texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
			texture.repeat.set(2, 2);
			shader.uniforms.diffuseMap.value = texture;
		});
		loader.load(normalPath, function (texture) {
			// texture.mapping = THREE.SphericalReflectionMapping;
			texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
			texture.repeat.set(2, 2);
			shader.uniforms.normalMap.value = texture;
		});
		loader.load(bumpPath, function (texture) {
			// texture.mapping = THREE.SphericalReflectionMapping;
			texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
			texture.repeat.set(2, 2);
			shader.uniforms.bumpMap.value = texture;
		});
		let geometry = new THREE.IcosahedronGeometry(80, 4);
		...
		for (let i = 0; i < 2000; i++) {
			let r = Math.random(), b = 1 - r;
			let star = new THREE.Vector3();
			let color = new THREE.Color(.8 + .2 * r, Math.max(r, b), .8 + .2 * b);
			var geopos = new Geopos().randomize(2000 + 1000 * r);
			star.copy(geopos.toVector());
			stars.vertices.push(star);
			stars.colors.push(color);
		}
		...
		starfield = new THREE.Points(stars, starsMaterial);
	...
	lights: function (world: World) {
		...
		for (let color of colors) {
			let light = new THREE.PointLight(color, 2, 0, 1);
	...
	render: function (delta: number = 0) {
		for (let light of lights) {
			light.geopos.setPosition(0.01, 0.01, light.position);
		}
